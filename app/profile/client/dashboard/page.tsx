...
export default function Page() {
  ...
  const jobPostings = [8, 12, 10, 15];
  const hiredFreelancers = [3, 5, 4, 6];
  const overviewData = weeks.map((w, i) => ({
    week: w,
    jobs: jobPostings[i],
    freelancers: hiredFreelancers[i],
    total: jobPostings[i] + hiredFreelancers[i],
  }));
  ...
  const pieData = [
    {
      ...
      value: jobPostings.reduce((a, b) => a + b, 0),
      color: "#3b82f6",
    },
    {
      label: "Design",
      value: hiredFreelancers.reduce((a, b) => a + b, 0),
      color: "#8b5cf6",
    },
    { label: "Marketing", value: 18, color: "#6366f1" },
  ];
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-purple-100 mt-1">
            Insights for your jobs, freelancers, and project management.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        {/* Right: Project Breakdown (Animated Donut) */}
        <div className="rounded-2xl border border-gray-200 bg-white p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900">
              Project Breakdown
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-3">
        ...
        <a
          href="/profile/client/review-proposals"
          className="inline-flex items-center gap-2 rounded-full bg-purple-600 text-white shadow-lg px-4 py-2 text-sm hover:bg-purple-700"
        >
          <MessageCircle className="h-4 w-4" /> Review Proposals
        </a>
        <a
          href="/profile/client/contracts"
          className="inline-flex items-center gap-2 rounded-full bg-indigo-600 text-white shadow-lg px-4 py-2 text-sm hover:bg-indigo-700"
        >
          <Handshake className="h-4 w-4" /> Manage Contracts
        </a>
      </div>
    </div>
  );
