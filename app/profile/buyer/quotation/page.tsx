  ...
  const loadAllInquiries = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log(`Buyer inquiries for user ${user.id}: ${buyerInquiries.length}`);
        console.log('Buyer inquiries:', JSON.stringify(buyerInquiries, null, 2));
        ...
        if (buyerInquiries.length > 0) {
          console.log('Sample buyer inquiry details:');
          buyerInquiries.forEach((inquiry, index) => {
            console.log(`Buyer Inquiry ${index + 1}:`, JSON.stringify(inquiry, null, 2));
            console.log(`- Inquiry By:`, inquiry.buyer?.id, 'vs Current User:', user.id);
          });
        }
      } else {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            View and manage your product inquiries and quotations
          </p>
        </div>
